...
internal static class AssetManager
{
    ...
    {
        ringID = span.Trim().ToString();
        if (ItemRegistry.GetData(ringID) is ParsedItemData data)
        {
            ringID = data.QualifiedItemId;
            return ringID is not null && data.Category == SObject.ringCategory;
        }
        return false;
    }
}
